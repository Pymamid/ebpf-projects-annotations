 ...
 OPENED COMMENT BEGIN 
{
  ...
  "humanFuncDescription": [
    {
      "description": "This function takes arguments as pointer to the struct bpf_elf_map, a pointer to the struct flow_infos and a pointer to the struct flow_tuple, and unsigned 32 bit integer op. Nothing is being returned from the function. The function take_snapshot takes the snapshot or stores the current state of the flow. When the function is invoked with a reason (which is passed to the function in the variable 'op'), a map lookup is performed to check if a previous snapshot for the st_map exists, if so the current state is firstly saved before moving ahead. A map lookup is performed with st_map and indices ranging from 0 to MAX_SNAPSHOTS-1 and stored in the curr_snapshot. The snapshot with the highest index signifies the latest snapshot. Fields pertaining to these such as sequence number,index etc is stored in arg.max_seq, arg.oldest_seq, arg.new_snapshot and arg.best_idx. If arg.new_snapshot is non-NULL, then the memory of the flow_info is copied into arg.new_snapshot->flow and the memory block of flow_id is stored into arg.new_snapshot->flow_id. This information along with the current system time, incremented sequence number and the reason is updated in the map.",
